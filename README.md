...
## 🆕 อัปเดตล่าสุด (รีสกิน + แก้บัค)

- **ธีม**: เปลี่ยนจาก glassmorphism (การ์ดโปร่งแสง + เบลอพื้นหลัง) เป็นพื้นหลัง/การ์ดแบบทึบ (solid/opaque) ทั้งเว็บ ยังคงโทนน้ำเงิน `#92bbff` เดิม แก้ได้ที่ `app/globals.css` (ตัวแปร `--bg`, `--card`)
- **ระบบตรวจสอบคีย์แยกออกมาเป็นของตัวเอง**: หน้า `/key` (กรอกคีย์หรือวางลิงก์) และ `/key/[code]` (ลิงก์ตรวจสอบตรง เปิดแล้วเห็นผลทันทีว่าเหลือกี่ชั่วโมง) — แยกจากหน้า `/orders` (ประวัติการซื้อ) โดยสมบูรณ์ ไม่ต้องล็อกอินก็เช็คได้ ใช้ API `/api/v1/key/[code]` เดิมที่มีอยู่แล้ว
- **แก้บัคหน้าโปรไฟล์**: รูปโปรไฟล์/แบนเนอร์ที่เคยซ้อนทับกัน (เพราะ margin ติดลบดึงรูปเข้าไปอยู่ในกล่องเดียวกับแบนเนอร์) ถูกแยกกล่องกันชัดเจนแล้ว ทั้งในหน้า `/profile` และ dropdown เมนูมุมขวาบน
- **กรอบรูป (avatar decoration/frame) ของ Discord**: เดิมโค้ดไม่เคยดึงข้อมูลนี้จาก Discord เลย (`lib/auth.ts` ดึงแค่ avatar กับ banner) ตอนนี้ดึง `avatar_decoration_data` มาด้วยแล้ว และแสดงผลผ่านคอมโพเนนต์ `components/Avatar.tsx` ตัวใหม่ (ใช้ร่วมกันทั้ง Navbar และหน้าโปรไฟล์)
- ต้องรัน `npx prisma db push` อีกครั้งหลังอัปเดต เพราะมีคอลัมน์ใหม่ `avatarDecoration` ใน `User`
...
{
  "valid": true,
  "product": "Flexozy Script Pro",
  "expiresAt": "2026-10-26T00:00:00.000Z",
  "hoursRemaining": 53,
  "minutesRemaining": 3180,
  "remainingLabel": "เหลือ 2 วัน 5 ชั่วโมง"
}
```

- `hoursRemaining` / `minutesRemaining` เป็น `null` ถ้าเป็นคีย์ตลอดชีพ (ไม่มีวันหมดอายุ) — ให้สคริปต์/โปรแกรมของคุณเช็ค `null` ก่อนเอาไปแสดงผล
- สำหรับ **คน** (ไม่ใช่สคริปต์) ที่อยากเช็คคีย์ด้วยตัวเอง ส่งลิงก์ `https://your-domain.vercel.app/key/FLEXOZY-XXXX-XXXX-XXXX-XXXX` ให้ลูกค้าได้เลย เปิดมาจะเห็นผลทันทีว่าเหลือกี่ชั่วโมง หรือจะให้กรอกเองที่หน้า `/key` ก็ได้
- `remainingLabel` เป็นข้อความภาษาไทยพร้อมใช้ เผื่อโปรแกรมอยากแสดงตรง ๆ โดยไม่ต้องคำนวณเอง
