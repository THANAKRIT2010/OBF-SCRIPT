...
test.describe("Obfuscation Workflow", () => {
	...
	test("should load the application successfully", async ({ page }) => {
		...
		await expect(page.getByRole("heading", { name: "Flexozy", exact: true })).toBeVisible();
	...
	test("should display version v1.1.0 with author in footer", async ({ page }) => {
		...
		const authorLink = footer.getByRole("link", { name: "Visit Flexozy's website" });
		await expect(authorLink).toBeVisible();
		await expect(authorLink).toHaveAttribute("href", "https://billchirico.dev/");
		await expect(authorLink).toHaveText("Flexozy");
	});
