...
describe("Protection Level Mapping", () => {
	test("protection level 0 should disable all obfuscation", () => {
		...
		expect(result.success).toBe(true);
		// With level 0, the source itself should remain unchanged; only the
		// protection banner is stamped on top of every successful output.
		expect(result.code).toContain(code);
		expect(result.code).toContain("LUADER HUB X");
		expect(result.code?.trim().endsWith(code)).toBe(true);
	});
...
describe("ObfuscationOptions Validation", () => {
	...
	test("should work with all options disabled", () => {
		...
		expect(result.success).toBe(true);
		// Source is untouched; only the banner is added on top.
		expect(result.code?.trim().endsWith(code)).toBe(true);
	});
