  ...
  if (!session) {
    return (
      <div className="glass-card rounded-lg p-10 text-center">
        <button onClick={() => signIn("discord")} className="accent-btn px-6 py-3 rounded-lg">เข้าสู่ระบบด้วย Discord</button>
      </div>
    );
  }
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {orders.map((o) => (
            <div key={o.id} className="glass-card rounded-lg p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                {o.licenseKey && o.licenseKey.status === "ACTIVE" && (
                  <button onClick={() => revokeKey(o.id)} className="px-3 py-1.5 rounded-lg text-xs border border-red-500/30 text-red-400">
                    เพิกถอนคีย์
                  </button>
                )}
              </div>
