...
import { obfuscateJavaScript } from "@/lib/obfuscator-js";
...
const DEFAULT_JS_CODE = `// Simple Inventory System
class Inventory {
  constructor(maxSlots = 20, playerName = "Player") {
    this.items = [];
  ...
  addItem(name, quantity = 1, rarity = "common") {
    if (this.items.length >= this.maxSlots) {
      return { success: false, message: "Inventory full" };
    }
    this.items.push({ name, quantity, rarity, timestamp: Date.now() });
    return { success: true, message: \`Added \${quantity}x \${name}\` };
  }

  sellItem(name, pricePerUnit) {
    ...
    return { success: true, message: \`Sold for \${total} gold\` };
  }
}

const playerInventory = new Inventory(20, "Hero");
playerInventory.addItem("Sword", 1, "rare");
playerInventory.addItem("Potion", 5, "common");

const result = playerInventory.sellItem("Potion", 10);
if (result.success) {
  console.log(\`Sold item for gold. New balance: \${playerInventory.gold}\`);
}
`;
...
export default function Home() {
	...
	const [codeLanguage, setCodeLanguage] = useState<"lua" | "javascript">("lua");
	...
	const handleLanguageChange = (lang: "lua" | "javascript") => {
		if (lang === codeLanguage) return;

		// Only swap the sample code if the user hasn't edited it away from the
		// current default - never overwrite code someone actually typed.
		if (lang === "javascript" && inputCode === DEFAULT_LUA_CODE) {
			setInputCode(DEFAULT_JS_CODE);
		} else if (lang === "lua" && inputCode === DEFAULT_JS_CODE) {
	...
	const obfuscateCode = async () => {
		...
		try {
			...
			const result: ObfuscationResult = await new Promise(resolve => {
				setTimeout(() => {
					if (codeLanguage === "javascript") {
						const jsResult = obfuscateJavaScript(inputCode, {
							protectionLevel: settings.compressionLevel,
						});
						// Normalize to the shared ObfuscationResult shape used by the UI below.
						resolve({ success: jsResult.success, code: jsResult.code, error: jsResult.error });
					} else {
			...
			if (result.success && result.code) {
				...
				// Record who obfuscated what, for the admin dashboard.
				fetch("/api/log", {
					method: "POST",
	...
	const downloadCode = () => {
		...
		a.href = url;
		a.download = codeLanguage === "javascript" ? "obfuscated.js" : "obfuscated.lua";
		a.click();
		...
		>
			<main className="absolute inset-0 z-10 flex flex-col p-4 sm:p-6 gap-4 lg:gap-6">
				...
				<header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 animate-in fade-in slide-in-from-top duration-700">
					<div className="flex items-center gap-4">
						<div className="relative group">
							...
							<div
								className="relative w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-slate-950/40 flex items-center justify-center shadow-2xl shadow-blue-500/30 ring-2 ring-white/20 backdrop-blur-sm transform group-hover:scale-105 transition-all duration-300 overflow-hidden"
							>
								<Image
									src="/logo.png"
									alt="Flexozy logo"
									width={56}
									height={56}
									className="w-10 h-10 sm:w-12 sm:h-12 object-contain drop-shadow-md group-hover:rotate-6 transition-transform duration-300"
									priority
								/>
							</div>
						</div>
						<div>
							...
							<p className="text-xs sm:text-sm text-gray-300/90 hidden sm:block font-medium">{t("tagline")}</p>
						</div>
					</div>
					<div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full sm:w-auto">
						{/* Account bar: language toggle, signed-in user, admin link, sign out */}
						<div className="flex items-center gap-2 flex-wrap">
							<div className="flex items-center rounded-lg border border-white/20 overflow-hidden text-xs font-semibold">
								<button
									...
									className={cn(
										...
										codeLanguage === "lua" ? "bg-white/20 text-white" : "text-gray-400 hover:text-white"
									)}
									aria-label="Switch to Lua"
								>
									Lua
								</button>
								<button
									...
									className={cn(
										"px-2.5 py-1.5 transition-colors",
										codeLanguage === "javascript" ? "bg-white/20 text-white" : "text-gray-400 hover:text-white"
									)}
									aria-label="Switch to JavaScript"
								>
									JS
								</button>
							</div>
							<div className="flex items-center rounded-lg border border-white/20 overflow-hidden text-xs font-semibold">
								<button
									...
									className={cn(
										"px-2.5 py-1.5 transition-colors",
										locale === "th" ? "bg-white/20 text-white" : "text-gray-400 hover:text-white"
									)}
								...
								<button
									...
									className={cn(
										"px-2.5 py-1.5 transition-colors",
										locale === "en" ? "bg-white/20 text-white" : "text-gray-400 hover:text-white"
									)}
							...
							{session?.user && (
								<div className="flex items-center gap-2 rounded-lg border border-white/20 bg-white/5 px-2.5 py-1.5">
									{session.user.image && (
										// eslint-disable-next-line @next/next/no-img-element
										<img src={session.user.image} alt="" className="w-5 h-5 rounded-full" />
									)}
									<span className="text-xs text-gray-200 max-w-[120px] truncate">{session.user.name}</span>
						...
						<nav className="flex flex-wrap gap-3 w-full sm:w-auto" aria-label="Main actions">
							...
							<Button
								...
								aria-label={codeLanguage === "javascript" ? "Obfuscate JavaScript code" : "Obfuscate Lua code"}
							>
								...
								<span className="relative z-10">{isProcessing ? t("processing") : t("obfuscate")}</span>
							</Button>
						</nav>
					</div>
				</header>
				...
				{showSuccessAnimation && (
					<div className="fixed top-20 right-6 z-50 animate-in slide-in-from-top fade-in duration-300">
						<div className="bg-gradient-to-r from-green-500/90 to-emerald-500/90 backdrop-blur-xl rounded-2xl px-6 py-4 shadow-2xl border border-green-400/30 flex items-center gap-3">
							...
							<div>
								<p className="text-white font-bold text-sm">{t("obfuscationComplete")}</p>
								<p className="text-green-50 text-xs">{t("codeProtected")}</p>
							</div>
						</div>
					</div>
				)}
								...
								<div className="p-4 border-b border-white/20 bg-gradient-to-r from-white/10 to-transparent backdrop-blur-sm">
									<div className="flex items-center gap-3">
										...
										<div>
											...
											<p className="text-xs text-gray-400 font-medium">{t("readyForObfuscation")}</p>
										</div>
									</div>
								</div>
									...
									<div className="flex items-center justify-between">
										<div className="flex items-center gap-3">
											...
											<div>
												...
												<p className="text-xs text-gray-400 font-medium">{t("protectedAndSecured")}</p>
											</div>
										</div>
										{outputCode && (
											<div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-500/30 backdrop-blur-sm animate-in fade-in zoom-in duration-500">
												...
												<span className="text-xs font-bold text-green-300">{t("active")}</span>
											</div>
										)}
									</div>
								</div>
								<div className="flex-1 min-h-0">
									<CodeEditor value={outputCode} readOnly language={codeLanguage} />
								</div>
							</Card>
						</section>
						...
						<aside className="lg:col-span-4 lg:overflow-auto" aria-labelledby="settings-heading">
							<Card className="bg-gradient-to-br from-background/60 via-background/50 to-background/40 backdrop-blur-2xl border-white/20 shadow-2xl shadow-black/30 p-6 ring-1 ring-white/10 hover:ring-white/20 transition-all duration-500">
								<div className="flex items-center gap-3 mb-6 pb-4 border-b border-white/20">
									<div className="relative">
										<div className="absolute inset-0 bg-gradient-to-br from-[#F7DF1E] to-[#F0DB4F] rounded-lg blur-md opacity-40"></div>
										<div className="relative w-10 h-10 rounded-lg bg-gradient-to-br from-[#F7DF1E] to-[#F0DB4F] flex items-center justify-center shadow-lg">
											<Settings className="w-5.5 h-5.5 text-slate-900" aria-hidden="true" />
										</div>
									</div>
									<div>
										...
										<p className="text-xs text-gray-400 font-medium">Powered by javascript-obfuscator</p>
									</div>
								</div>

								<div className="space-y-4">
									<div className="flex items-center justify-between">
										<Label className="text-sm font-semibold text-gray-200">Protection Level</Label>
										<span className="text-sm font-bold text-blue-400">{settings.compressionLevel}%</span>
									</div>
									<Slider
										value={[settings.compressionLevel]}
										onValueChange={([level]) => setSettings({ ...settings, compressionLevel: level })}
										min={0}
										max={100}
										step={10}
										className="w-full"
									...
									<p className="text-xs text-gray-400 leading-relaxed">
										Higher levels enable more transformations at once: identifier renaming, string array
										extraction with RC4/base64 encoding, control-flow flattening, dead-code injection, string
										splitting, self-defending code, and console/debugger blocking. Very high settings produce
										larger and slower output in exchange for stronger protection.
									</p>
								</div>
							</Card>
						</aside>
				...
				<section className="sr-only">
					<h2>About Flexozy</h2>
					<p>
						Flexozy is a professional, free online tool for protecting Lua source code through advanced
					...
					<p>
						Your code security and privacy are our top priorities. Flexozy runs entirely in your web
					...
					<dl>
						...
						<dd>
							While no obfuscation is 100% irreversible, Flexozy makes reverse engineering significantly
					...
					<p>
						Flexozy is built with modern web technologies including Next.js 15, React 19, TypeScript, and
					...
					<h3>Open Source</h3>
					<p>
						Flexozy is open source and available on GitHub. You can review the code, contribute
				...
				>
					<div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 backdrop-blur-sm border border-white/10 hover:bg-white/10 transition-all duration-300">
						...
						<a
							...
							aria-label="Visit Flexozy's website"
						>
							Flexozy
						</a>
					</div>
