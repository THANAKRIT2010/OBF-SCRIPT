import { computeSafeLineInjectionPoints } from "./safe-injection";
...
export function injectAntiDebugChecks(
	...
	// Only inject right after a function-declaration line when: the
	// parameter list is fully closed on that same line (net bracket depth
	// is 0, so it's not a multi-line signature), and it isn't a one-liner
	// like `local function f() return 1 end` where "end" already closes the
	// body on the same line - injecting after that would put the check
	// outside the function entirely.
	const safePoints = computeSafeLineInjectionPoints(code);

	for (let i = 0; i < lines.length; i++) {
		result.push(lines[i]);

		const trimmed = lines[i].trim();
		const isFunctionDecl = trimmed.startsWith("function ") || trimmed.startsWith("local function ");
		const isOneLiner = /\bend\b\s*$/.test(trimmed) || /\)\s*end\b/.test(trimmed);
		const parenBalanced = (trimmed.match(/\(/g) || []).length === (trimmed.match(/\)/g) || []).length;

		if (isFunctionDecl && !isOneLiner && parenBalanced && safePoints[i]) {
