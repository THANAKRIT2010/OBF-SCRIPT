...
export function injectDeadCode(code: string, injectionRate: number = 50): string {
	...
	// Only ever splice a new statement in after a line that is confirmed to
	// be a safe top-level statement boundary: not inside a multi-line
	// string/comment, not inside an open (), {}, [], and not a line that
	// continues onto the next one (trailing operator, comma, "and", etc.).
	// This is what previously let dead-code injection land inside a
	// multi-line table constructor or long string and corrupt the syntax.
