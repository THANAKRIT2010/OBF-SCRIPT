...
export function encryptXOR(input: string): EncryptedString {
	...
	// IMPORTANT: uses only arithmetic (no ~, <<, >>, &, |) so it runs on
	// Lua 5.1 / LuaJIT (WoW, FiveM, Garry's Mod, most Roblox executors) as
	// well as Lua 5.3+. The native bitwise operators are Lua 5.3+ only and
	// were the main reason obfuscated output stopped running once XOR
	// encryption kicked in at 70%+ protection.
	const decryptorFunction = `(function(t,k)local function bx(a,b)local r,p=0,1 while a>0 or b>0 do local ab,bb=a%2,b%2 if ab~=bb then r=r+p end a=(a-ab)/2 b=(b-bb)/2 p=p*2 end return r end local s="" for i=1,#t do s=s..string.char(bx(t[i],((k+i-1)%255)+1)) end return s end)`;
...
export function encryptBase64(input: string): EncryptedString {
	...
	// Rewritten with pure arithmetic (floor division / modulo) instead of
	// <<, >>, &, | so it works on Lua 5.1/5.2/LuaJIT, not just Lua 5.3+.
