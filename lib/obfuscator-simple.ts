...
/**
 * Banner injected at the very top of every successfully obfuscated file.
 * Kept as a plain top-of-file comment (not run through minify/format's
 * comment-stripping), so it's always readable, regardless of formatting style.
 */
const PROTECTION_BANNER = `--[[
██╗     ██╗   ██╗ █████╗ ██████╗ ███████╗██████╗     ██╗  ██╗██╗   ██╗██████╗     ██╗  ██╗
██║     ██║   ██║██╔══██╗██╔══██╗██╔════╝██╔══██╗    ██║  ██║██║   ██║██╔══██╗    ╚██╗██╔╝
██║     ██║   ██║███████║██║  ██║█████╗  ██████╔╝    ███████║██║   ██║██████╔╝     ╚███╔╝
██║     ██║   ██║██╔══██║██║  ██║██╔══╝  ██╔══██╗    ██╔══██║██║   ██║██╔══██╗     ██╔██╗
...
export class LuaObfuscator {
	...
	obfuscate(
		...
		try {
			...
			// Safety net: verify the transformed code is still syntactically
			// valid Lua before returning it. The line-based passes (dead
			// code injection, anti-debug injection) are now guarded against
			// the known unsafe cases, but this catch-all guarantees the user
			// never receives broken output even from an edge case we
			// haven't anticipated - instead we automatically fall back to a
			// safer configuration and retry once.
			const finalCheck = parseLua(obfuscatedCode);
			if (!finalCheck.success) {
				if (!_isRetry && (options.deadCodeInjection || options.antiDebugging)) {
				...
				return {
					...
					error:
						finalCheck.error ||
						"Obfuscation produced invalid Lua output; try a lower protection level or disable dead code / anti-debug injection.",
					errorDetails: finalCheck.errorDetails,
				};
			}
	...
	private mangleNamesRegex(code: string): string {
		...
		const protectedNames = new Set([
			...
			"sqrt",
			"tan",
			// Runtime/engine globals used by real-world Lua targets (Roblox,
			// WoW, FiveM/Garry's Mod, general Lua 5.1). None of these are
			// declared anywhere in the user's script, so the old mangler had
			// no way to know they're "protected" - it just renamed every
			// bare identifier it saw, which broke any script that touched
			// the engine API (e.g. `game:GetService(...)`, `wait(1)`).
			"game",
			"workspace",
			"script",
			"shared",
			...
			"utf8",
			"buffer",
			// Common globals in WoW/FiveM addons
			"CreateFrame",
			"GetTime",
			"UnitName",
			"SlashCmdList",
		...
		// Guard against mangling identifier-like text that lives inside string
		// literals (e.g. a URL, a table key referenced as a string, log text
		// that happens to match a variable name). Without this, the naive
		// \bname\b replacement below runs over the *entire* source text and
		// silently corrupts string contents anywhere they match a variable name.
		const stringPattern = /(["'])(?:(?=(\\?))\2.)*?\1/g;
		const placeholders: string[] = [];
		const placeholderPrefix = "__OBFSTR_PLACEHOLDER_";
		const codeNoStrings = code.replace(stringPattern, literal => {
		...
		// IMPORTANT: we deliberately do NOT collect identifiers that appear
		// after a "." or ":" (member/method access, e.g. `obj.Health`,
		// `player:Kick()`). This regex-based mangler has no scope/type
		// info, so it can't tell a local variable from a field name on an
		// external table (a Roblox Instance, a WoW API table, etc). Renaming
		// those breaks the script at runtime with "attempt to call a nil
		// value" since the mangled name no longer matches the real API.
		// Skipping member access is a conservative fix: it means field/
		// method names are never mangled, but it guarantees calls into
		// external APIs keep working at every protection level.
		const identifierPattern = /(?<![.:])\b([a-zA-Z_][a-zA-Z0-9_]*)\b/g;
		const identifiers = new Set<string>();

		let match;
		while ((match = identifierPattern.exec(codeNoStrings)) !== null) {
			const name = match[1];
			if (!protectedNames.has(name) && !name.startsWith(placeholderPrefix)) {
		...
		// Replace all occurrences, but still never touch a name that's
		// preceded by "." or ":" (see comment above) even though such
		// occurrences were never added to nameMap as declarations - a
		// variable can be both a local (`local Health = 100`) and, by
		// coincidence, a field name elsewhere (`obj.Health`); only the
		// non-member-access occurrences should be renamed.
		let result = codeNoStrings;
		this.nameMap.forEach((mangledName, originalName) => {
			const regex = new RegExp(`(?<![.:])\\b${originalName}\\b`, "g");
			result = result.replace(regex, mangledName);
		});

		// Restore the original string literals, untouched
		result = result.replace(new RegExp(`${placeholderPrefix}(\\d+)__`, "g"), (_, idx) => placeholders[Number(idx)]);
...
export function obfuscateLua(code: string, options?: ObfuscationOptions): ObfuscationResult {
	...
	const result = obfuscator.obfuscate(code, finalOptions);

	// Stamp every successfully obfuscated output with the protection banner.
	if (result.success && result.code) {
		result.code = `${PROTECTION_BANNER}\n\n${result.code}`;
	}
